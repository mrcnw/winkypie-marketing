...
app/public/assets/winkypie/instagram/     → *Pinned tiles* on **both** /winkypie · Assets and
  pinned/                                   /instagram · Profile — the same paired cards from
    upload/                                 one component, so a tile is never listed twice.
                                            `x.txt` beside `x.png` is the post caption, pasted
                                            as-is. `pinned/x.png` is the master; `pinned/
                                            upload/x.jpg` is what gets posted, written by
                                            `npm run ig` and never by hand. A lane other than
                                            `pinned/` falls through to a plain gallery on
                                            /winkypie and is not shown on /instagram
...
`Text card` — `UGC` was dropped 2026-09-24 and an old `UGC` cell now renders uncoloured) — a new pillar needs a key there and a colour token in `globals.css`
